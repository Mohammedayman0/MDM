...
def _ensure_table(all_tables, name, alias, is_cte):
    if name not in all_tables:
        all_tables[name] = {
            ...
            "is_cte": is_cte,
            "columns": [],
            "refs": [],  # columns referenced outside the SELECT list (WHERE / JOIN ON / etc.)
        }
...
def _collect_clause_refs(stmt, alias_map, all_tables, cte_names):
    """Capture column references from WHERE / JOIN...ON / GROUP BY / HAVING / ORDER BY."""

    def register(node, clause_label):
        if node is None:
            return
        for col in node.find_all(exp.Column):
            ...
            table_entry = _ensure_table(all_tables, target, "", target.upper() in cte_names)
            ref_key = (clause_label, col_name)
            existing = {(r["clause"], r["column"]) for r in table_entry["refs"]}
            if ref_key not in existing:
                table_entry["refs"].append({"clause": clause_label, "column": col_name})

    for where in stmt.find_all(exp.Where):
        register(where, "WHERE")
    for join in stmt.find_all(exp.Join):
        register(join.args.get("on"), "JOIN ON")
    for group in stmt.find_all(exp.Group):
        register(group, "GROUP BY")
    for having in stmt.find_all(exp.Having):
        register(having, "HAVING")
    for order in stmt.find_all(exp.Order):
        register(order, "ORDER BY")
...
def parse_sql(sql_text: str, dialect: str | None = None) -> dict:
    ...
    Returns:
      {
        ...
        "tables": {
            TABLE_NAME: {
                ...
                refs: [{clause, column}],   # refs found outside the SELECT list
            }
        },
        "errors": [str],
      }
    ...
    for stmt in statements:
        ...
        # Register every real table in the statement, even ones that only
        # ever appear in a WHERE/JOIN condition and never in a SELECT list.
        for table in stmt.find_all(exp.Table):
            name = (table.name or "").upper()
            alias = (table.alias or "").upper()
            if not name:
        ...
        # Capture anything referenced only in WHERE / JOIN ON / GROUP BY / HAVING / ORDER BY.
...
if "result" in st.session_state and st.session_state["result"]:
    ...
    # Tabs
    tab1, tab2, tab3 = st.tabs(["📋  Detailed View", "🔁  Synonyms", "📊  Full Table"])
        ...
        for tname, info in tables.items():
            ...
            with st.expander(label, expanded=True):
                ...
                if info["refs"]:
                    ...
                    ref_badges = " ".join(
                        f'<span class="badge badge-ref">{r["clause"]}: {r["column"]}</span>' for r in info["refs"]
                    )
                    st.markdown(ref_badges, unsafe_allow_html=True)
