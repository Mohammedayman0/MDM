        ...
        with st.spinner("🤖 Analyzing..."):
            try:
                ...
                response = client.messages.create(
                    ...
                    max_tokens=800,
                    system=f"""You are a Data Quality Analyst. Analyze this dataset and give clear, concise insights.
Dataset: {total_rows:,} rows, {total_cols} columns. Completeness: {completeness}%. Duplicate rows: {dup_rows:,}.
Columns: {'; '.join(col_summaries[:8])}
Be specific and practical. Max 200 words. Use bullet points.""",
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if total_matches == 0:
        ...
        with tab_raw:
            # Build flat per-record table: Row No | Name | Code | Match Group ID | Status | Similarity
            name_col_display = st.session_state.get("name_col1", "Name")
            flat_rows = []
            seen = {}
            for i, cl in enumerate(clusters):
                grp_code = f"GRP-{i+1:03d}"
                golden = pick_golden_record(cl["names"])
                for name in cl["names"]:
                    # Find original row number in df1
                    if df1 is not None:
                        mask = df1[name_col_display].astype(str) == str(name)
                        row_indices = df1[mask].index.tolist()
                        row_no = row_indices[0] + 2 if row_indices else "—"
                    else:
                        row_no = "—"
                    # Find max similarity score for this record
                    score_col = "Avg Score"
                    a_cols = [c for c in results.columns if c.startswith("A:")]
                    b_cols = [c for c in results.columns if c.startswith("B:")]
                    rec_scores = []
                    if a_cols and b_cols:
                        mask_a = results[a_cols[0]].astype(str) == str(name)
                        mask_b = results[b_cols[0]].astype(str) == str(name)
                        rec_scores = results[mask_a | mask_b][score_col].tolist()
                    max_sim = max(rec_scores) if rec_scores else cl["max_score"]
                    status = "Exact Match" if max_sim == 100 else "Similar"
                    is_golden = "🏅 Golden" if name == golden else "Duplicate"
                    flat_rows.append({
                        "Row No": row_no,
                        name_col_display: name,
                        "Code": is_golden,
                        "Match Group ID": grp_code,
                        "Status": status,
                        "Similarity %": max_sim
                    })
            ...
            with cf2: search = st.text_input("🔍 Search:", placeholder="Type a name or group...")
            ...
            def style_row(row):
                styles = [""] * len(row)
                if row.get("Code") == "🏅 Golden":
                    styles = ["background-color:#fef9c3;font-weight:700"] * len(row)
                elif row.get("Similarity %", 0) == 100:
                    styles[list(row.index).index("Similarity %")] = "color:#dc2626;font-weight:700"
                return styles

            st.dataframe(disp, use_container_width=True, height=420)
            st.caption(f"Showing {len(disp):,} records across {len(clusters):,} match groups")
        ...
        out = io.BytesIO()
        with pd.ExcelWriter(out, engine='openpyxl') as writer:
            flat_df.to_excel(writer, sheet_name='Match Results', index=False)
            cl_rows = []
            for i, cl in enumerate(clusters):
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if st.button("✅ Apply Merge & Export Clean File", use_container_width=True):
        merged = df1.copy()
        # Step 1: replace all cluster members with their golden record
        for i, cluster in enumerate(clusters):
            golden = st.session_state.golden_overrides.get(f"golden_{i}", pick_golden_record(cluster["names"]))
            for name in cluster["names"]:
                if name != golden:
                    merged.loc[merged[name_col1].astype(str) == str(name), name_col1] = golden
        # Step 2: deduplicate — keep only unique golden records (drop duplicate rows)
        merged_unique = merged.drop_duplicates(subset=[name_col1])
        st.session_state.merged_df = merged_unique
        original_unique = df1[name_col1].nunique()
        after_unique = merged_unique[name_col1].nunique()
        removed = len(merged) - len(merged_unique)
        st.success(f"✅ Merge complete! {len(merged_unique):,} unique golden records.")
        st.markdown(f"""
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin:16px 0">
            <div class="metric-card"><div class="val">{len(df1):,}</div><div class="lbl">Original Records</div></div>
            <div class="metric-card"><div class="val" style="color:#16a34a">{len(merged_unique):,}</div><div class="lbl">Unique After Merge</div></div>
            <div class="metric-card"><div class="val" style="color:#2563eb">{original_unique - after_unique:,}</div><div class="lbl">Duplicates Resolved</div></div>
            <div class="metric-card"><div class="val" style="color:#d97706">{removed:,}</div><div class="lbl">Rows Removed</div></div>
        </div>""", unsafe_allow_html=True)
        st.dataframe(merged_unique.head(20), use_container_width=True)

    if st.session_state.get("merged_df") is not None:
        merged_unique = st.session_state.merged_df
        out_m = io.BytesIO()
        with pd.ExcelWriter(out_m, engine='openpyxl') as writer:
            # Sheet 1: unique golden records only
            merged_unique.to_excel(writer, sheet_name='Golden Records (Unique)', index=False)
            # Sheet 2: merge map
            cl_rows = []
            for i, cl in enumerate(clusters):
                ...
                for name in cl["names"]:
                    cl_rows.append({
                        ...
                        "Golden Record": g,
                        "Merged": "Yes" if name != g else "Golden"
                    })
            pd.DataFrame(cl_rows).to_excel(writer, sheet_name='Merge Map', index=False)
        out_m.seek(0)
        st.download_button("⬇️ Download Clean File — Unique Golden Records (Excel)", data=out_m,
                ...
                with st.spinner("🤖 Analyzing match results..."):
                    try:
                        ...
                        response = client.messages.create(
                            model="claude-sonnet-4-20250514",
                            max_tokens=800,
                            system=f"""You are an MDM consultant. Analyze these deduplication results concisely.
{match_summary}
Be specific with numbers. Max 200 words. Use bullet points.""",
                            messages=msg_payload
                        )
                        ai_text = response.content[0].text if response.content else "No response."
                        st.session_state[match_chat_key].append({"role": "assistant", "content": ai_text})
                        st.rerun()
                    except Exception as e:
                        st.session_state[match_chat_key].append({"role": "assistant", "content": f"⚠️ Error: {str(e)[:120]}"})
                        st.rerun()
