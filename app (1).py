...
if step == 1:
    ...
    with col_main:
        ...
        if st.session_state.mode == "within":
            ...
            file1 = st.file_uploader("", type=["xlsx", "xls", "csv"], key="f1", label_visibility="collapsed")
            ...
            if file1:
                ...
                st.markdown("<br>", unsafe_allow_html=True)
                if st.button("التالي ← الإعدادات", use_container_width=True):
            ...
            file2 = st.file_uploader("", type=["xlsx", "xls", "csv"], key="f2", label_visibility="collapsed")
            if file1 and file2:
                df1 = pd.read_csv(file1) if file1.name.endswith(".csv") else pd.read_excel(file1)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with col_main:
        ...
        with b2:
            if st.button("🚀 ابدأ المطابقة", use_container_width=True):
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if total_matches == 0:
        ...
        with tab_raw:
            ...
            st.dataframe(disp.style.applymap(hl, subset=["نسبة التشابه"]),
        ...
        with tab_clusters:
            ...
            """, unsafe_allow_html=True)
            show_limit = st.slider("عدد المجموعات المعروضة:", 5, min(100, len(clusters)), min(20, len(clusters)), key="cl_limit")
    ...
    with nav1:
        if st.button("← تعديل الإعدادات", use_container_width=True):
            st.session_state.results_df = None
            st.session_state.clusters = None
            st.session_state.step = 2; st.rerun()
    with nav2:
        if total_matches > 0:
            if st.button("🏅 الانتقال للدمج — Golden Record Merge", use_container_width=True):
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    show_n = st.slider("عدد المجموعات:", 5, min(100, len(clusters)), min(20, len(clusters)), key="merge_limit")
    ...
    if st.button("✅ تطبيق الدمج وتصدير الملف النظيف", use_container_width=True):
        merged = df1.copy()
    ...
    b1, b2 = st.columns(2)
    with b1:
        if st.button("← رجوع للنتائج", use_container_width=True):
